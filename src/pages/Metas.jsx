...
export default function Metas({ usuarioLogado, onVoltar }) {
  ...
  const [editMetas,   setEditMetas]   = useState({}) // { fiscal_login: valor_meta }
  ...
  const carregar = async () => {
    ...
    try {
      ...
      // Carrega metas do mês
      const { data: mData } = await supabase
        .from('metas_fiscal').select('*')
        .eq('mes_ano', mesAno)
      setMetas(mData || [])

      // Carrega auditorias realizadas no mês
      const [ano, mes] = mesAno.split('-')
      ...
      // Inicializa o mapa de edição com as metas carregadas
      const mapaEdit = {}
  ...
  const salvarMetas = async () => {
    ...
    setMsgSalvo('')
    try {
      for (const [fiscal_login, meta] of Object.entries(editMetas)) {
        await supabase.from('metas_fiscal').upsert(
          { fiscal_login, mes_ano: mesAno, meta: parseInt(meta) || 0 },
          { onConflict: 'fiscal_login,mes_ano' }
        )
      }
      ...
      setMsgSalvo('❌ Erro ao salvar: ' + e.message)
    } finally {
      setSalvando(false)
    }
  }

  // Monta dados consolidados por fiscal
  ...
  }).filter(f => f.meta > 0 || f.total > 0) // esconde fiscais sem meta e sem auditoria
  ...
  // Meses para navegar (últimos 6 + próximo)
    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
            <div>
              ...
              <p style={{ fontSize: 12, opacity: 0.8, marginTop: 3 }}>
                Cadastro e acompanhamento de metas mensais
              </p>
            </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '16px 16px 80px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {msgSalvo && (
            <p style={{ marginTop: 20, fontSize: 13, fontWeight: 700,
              color: msgSalvo.startsWith('✅') ? '#15803d' : '#dc2626' }}>
              {msgSalvo}
            </p>
          )}
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e2e8f0', padding: '16px', marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: '#374151' }}>
              Progresso Geral — {mesLabel(mesAno)}
            </span>
        ...
        {loading ? (
          ...
          <>
            {/* Modo editar — tabela simples para definir metas */}
            {modoEditar && (
            ...
            {dadosFiscais.length === 0 && !modoEditar ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {dadosFiscais
                  .sort((a, b) => b.pct - a.pct) // Ordena por % cumprimento
                  .map(f => (
                    <div key={f.login} style={{
                      ...
                      <div style={{ display: 'flex', gap: 12, fontSize: 12, color: '#64748b', flexWrap: 'wrap' }}>
                        ...
                        <span>📊 Nota média: <strong>{f.notaMedia}</strong></span>
                        {f.faltam > 0 && (
