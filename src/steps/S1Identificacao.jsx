...
// Busca colaboradores pelo prefixo selecionado
...
// Busca colaboradores por nome (digitação livre)
...
function AutocompleteInput({ label, value, onChange, onSelect, suggestions, placeholder, required, info }) {
  ...
  return (
    <div className="form-group" style={{ position: 'relative' }} ref={ref}>
      ...
      />
      {info && <p style={{ fontSize: 11, color: '#64748b', marginTop: 3 }}>{info}</p>}
...
export default function S1Identificacao({ form, upd, setForm, next, prev }) {
  ...
  const [eletricistas,  setEletricistas]  = useState([]) // lista da equipe pelo prefixo

  const ok = form.fiscal && form.matricula && form.prefixo && form.os && form.uc && form.lat

  // Quando prefixo muda, carrega eletricistas da equipe
  useEffect(() => {
  ...
  const onElet1Change = async v => {
    ...
    if (eletricistas.length > 0) {
      setElet1Sugs(
        eletricistas
          .filter(e => e.colaborador.toUpperCase().includes(v.toUpperCase()))
          .map(e => `${e.colaborador} (${e.matricula})`)
      )
    } else {
  ...
  return (
    <div>
      ...
      <button onClick={capturarGPS} disabled={gpsLoading} style={{
        ...
        {gpsLoading ? '📡 Capturando GPS...' : form.lat ? `📍 GPS capturado — ${form.lat}, ${form.lng}` : '📍 Capturar GPS e Endereço Automático'}
      </button>
